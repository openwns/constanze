    ...
    currentWindowEnd(windowSize)
{
    ...
    wns::probe::bus::ContextProviderCollection localContext(
        &component->getContextProviderCollection());
    localContext.addProvider(
        wns::probe::bus::contextprovider::Container(&this->bitsWindowedResult));
    localContext.addProvider(
        wns::probe::bus::contextprovider::Container(&this->packetWindowedResult));
	...
	MESSAGE_SINGLE(NORMAL, log, "Listener created.");
}
...
Listener::~Listener()
{
	...
	m << " at my IP=" << *binding; // uses ::printAddress()
	MESSAGE_END();
...
void Listener::onData(const wns::osi::PDUPtr& _data)
{
	...
	// easy throughput probes (they converge to the end value=average):
	//if (bitsRatePrId)	pdb->put(bitsRatePrId, static_cast<double>(bitCounter) / now);
	//if (packetRatePrId) pdb->put(packetRatePrId, static_cast<double>(packetCounter) / now);
	// window mechanism:
	...
	while (now >= currentWindowEnd) { // need to shift window
		...
		//MESSAGE_BEGIN(NORMAL, log, m, "window probe ");
		//m << "in ["<< currentWindowEnd-windowSize <<"..."<< currentWindowEnd <<"] = "<<static_cast<double>(windowedPacketCounter) / windowSize;
		//MESSAGE_END();
